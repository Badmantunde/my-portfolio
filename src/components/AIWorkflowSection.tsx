  ...
  return (
    ...
    >
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12">
        ...
        <AnimateIn variant="scale-up" delay={100} className="relative bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-10 lg:p-14 border border-gray-100 shadow-[0_2px_24px_rgba(0,0,0,0.04)]">
          ...
          <AnimateIn variant="fade-in" className="relative flex flex-col items-center mb-10 sm:mb-12">
          ...
          <div className="relative grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            {workflowTools.map((tool, i) => (
              <AnimateIn
                ...
                variant="scale-up"
                delay={i * 50}
                className="group flex flex-col items-center justify-center bg-[#F5F5F5] hover:bg-[#EDF5F0] rounded-xl p-4 sm:p-5 transition-colors duration-300 hover-lift"
              >
          ...
          <div className="relative mt-8 sm:mt-10 flex flex-wrap justify-center gap-2 sm:gap-3">
            {['Agentic workflows', 'Rapid MVP', 'AI prototyping', 'Multi-agent dev'].map(
              (tag, i) => (
                <AnimateIn key={tag} variant="fade-in" delay={i * 80} as="span" className="text-[11px] sm:text-[12px] font-medium text-gray-600 border border-gray-200 rounded-full px-3 py-1">
                  {tag}
                </AnimateIn>
              ),
            )}
